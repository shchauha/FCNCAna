...
int ScanChain(TChain *ch, TString options="", TString outputdir="outputs"){
  ...
  bool doExp = options.Contains("doExp");
  ...
  HistCol h_njets         (regions, "njets"           , 5 , 0   , 5   ,  &registry);
  HistCol h_njets40       (regions, "njets40"         , 5 , 0   , 5   ,  &registry);
  HistCol h_nisrjets      (regions, "nisrjets"        , 5 , 0   , 5   ,  &registry);
  HistCol h_nisrmatch     (regions, "nisrmatch"       , 5 , 0   , 5   ,  &registry);
  HistCol h_nlb40         (regions, "nlb40"           , 3 , 0   , 3   ,  &registry);
  HistCol h_ntb40         (regions, "ntb40"           , 3 , 0   , 3   ,  &registry);
  HistCol h_nbtags        (regions, "nbtags"          , 3 , 0   , 3   ,  &registry);
  HistCol h_nbtagstail    (regions, "nbtags"          , 3 , 0   , 3   ,  &registry);
  HistCol h_nbtagsM       (regions, "nbtagsM"         , 3 , 0   , 3   ,  &registry);
  HistCol h_nbtags25      (regions, "nbtags25"        , 3 , 0   , 3   ,  &registry);
  HistCol h_nbtags25M     (regions, "nbtags25M"       , 3 , 0   , 3   ,  &registry);
  ...
  float tree_l3ptratio = -1;
  float tree_l3ptrel = -1;
  float tree_weight = -1;
  float tree_jet3pt = -1;
  ...
  out_tree->Branch("mll13", &tree_mll13 );
  out_tree->Branch("mll23", &tree_mll23 );
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
	...
	// if (!isData && ss::lep1_motherID()==2) continue;
	...
	if (hyp_class == 6) {
	  ...
	  if (!foundGoodLoose)
	    continue;
	  // subtract double Fake
	  if (hyp_class == 1 && lep1pt>min_pt_fake && lep2pt>min_pt_fake) weight *= -1.;

	  // subtract prompt MC 
	  // if (isData and !ss::is_real_data()) weight *= -1.; //(turn if off when running on MC only)
	...
	//cout<<" bdt value"<<reader_hut->EvaluateMVA("BDTG method")<<endl;
      ...
      if( hyp_class == 3 and nleps == 2 and njets >= 2 and nbtags > 0 and lep1ccpt > 25 and lep2ccpt > 20 and faketype == 1 ) fill_region("ssbrfakeel", weight); 
      

      if( hyp_class == 3 and nleps == 2 and njets >= 2 and nbtags > 0 and lep1ccpt > 25 and lep2ccpt > 20 and faketype == 2 ) fill_region("ssbrfakemu", weight); 
	...
	//cout<< "nlep, nj, nb, sr "<<nleps<<" "<<njets<<" "<<nbtags<<" "<<sr_fcnc<<endl;	
      }
      ...
      if (hyp_class == 6 and nleps >  2 and njets >= 1 and nbtags >  0 and lep1ccpt > 25 and lep2ccpt > 20 and lep3ccpt > 20 ) fill_region("mlbronz", weight);        
      //if (hyp_class == 6 and nleps >  2 and njets >= 1 and nbtags >  0 and lep1ccpt > 25 and lep2ccpt > 20 and lep3ccpt > 20 ) fill_region("mlbronz", weight);             
